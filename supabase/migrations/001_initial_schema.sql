...
-- This migration creates all tables and RLS policies for the Coinvest platform
...
CREATE POLICY "idea_holders_update_own_ideas" ON ideas
  FOR UPDATE
...
-- ============================================================================
-- CHAT ROOM ATTENDEES RLS POLICIES
-- ============================================================================
...
CREATE POLICY "users_see_meetings_via_chatroom" ON meetings
  ...
  USING (
    EXISTS (
      ...
      JOIN chat_room_attendees cra ON cr.id = cra.chat_room_id
...
-- ============================================================================
-- PITCH REQUESTS RLS POLICIES
-- ============================================================================

CREATE POLICY "idea_holders_read_own_requests" ON pitch_requests
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM ideas
      WHERE ideas.id = pitch_requests.idea_id
      AND ideas.owner_id = auth.uid()
    )
  );

CREATE POLICY "investors_read_own_requests" ON pitch_requests
  FOR SELECT
  USING (auth.uid() = investor_id);

CREATE POLICY "admin_read_all_requests" ON pitch_requests
  FOR SELECT
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));

CREATE POLICY "investors_create_requests" ON pitch_requests
  FOR INSERT
  WITH CHECK (
...
CREATE POLICY "investors_read_own_votes" ON task_votes
  FOR SELECT
  USING (auth.uid() = investor_id);
...
CREATE POLICY "users_read_own_agreements" ON agreements
  FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "users_manage_own_agreements" ON agreements
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "admin_read_all_agreements" ON agreements
  FOR SELECT
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin'));
